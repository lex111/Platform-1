...
class PublicActionTest extends BrowserKitTest
{
    public function test_app_not_public()
    {
        ...
        $book = \DocsPen\Book::orderBy('name', 'asc')->first();
    ...
    public function test_books_viewable()
    {
        ...
        $books = \DocsPen\Book::orderBy('name', 'asc')->take(10)->get();
        $bookToVisit = $books[1];
    ...
    {
        $this->setSettings(['app-public' => 'true']);
        $chapterToVisit = \DocsPen\Chapter::first();
    ...
    {
        $this->setSettings(['app-public' => 'true']);
        $publicRole = \DocsPen\Role::getSystemRole('public');
        // Grant all permissions to public
        $publicRole->permissions()->detach();
        foreach (\DocsPen\RolePermission::all() as $perm) {
            $publicRole->attachPermission($perm);
        }
        $this->app[\DocsPen\Services\PermissionService::class]->buildJointPermissionForRole($publicRole);
