...
class RolesTest extends BrowserKitTest
{
    ...
    {
        $otherUsersPage = \DocsPen\Page::first();
    ...
    public function test_page_create_own_permissions()
    {
        ...
        foreach ($accessUrls as $index => $url) {
            $this->actingAs($this->user)->visit($url);
            $expectedUrl = \DocsPen\Page::where('draft', '=', true)->orderBy('id', 'desc')->first()->getUrl();
    ...
    public function test_role_permission_removal()
    {
        ...
        $page = Page::first();
        $viewerRole = \DocsPen\Role::getRole('viewer');
    ...
    public function test_empty_state_actions_not_visible_without_permission()
    {
        $admin = $this->getAdmin();
        // Book links
        $book = factory(\DocsPen\Book::class)->create(['created_by' => $admin->id, 'updated_by' => $admin->id]);
        ...
        // Chapter links
        $chapter = factory(\DocsPen\Chapter::class)->create(['created_by' => $admin->id, 'updated_by' => $admin->id, 'book_id' => $book->id]);
