...
let setupPageShow = (window.setupPageShow = function(pageId) {
  ...
  $(document.body)
    ...
    .on("click focus", event => {
      ...
      if (
        target.is(".zmdi") ||
        $(event.target).closest("#pointer").length === 1
      )
        return;
  ...
  function updatePointerContent() {
    let inputText = pointerModeLink
      ? window.baseUrl(`/link/${pageId}#${pointerSectionId}`)
      : `{{@${pageId}#${pointerSectionId}}}`;
    if (pointerModeLink && inputText.indexOf("http") !== 0)
      inputText =
  ...
  $('.page-content [id^="bkmrk"]').on("mouseup keyup", function(e) {
    ...
    let pointerLeftOffset =
      e.pageX - $elem.offset().left - $pointerInner.width() / 2;
  ...
  let pageScrollable =
    ...
    $sidebar.height() < $(".page-content").height();
  ...
  function addPageHighlighting() {
    ...
    function addNavObserver(headings) {
      ...
      let pageNavObserver = new IntersectionObserver(
    ...
    function toggleAnchorHighlighting(elementId, shouldHighlight) {
      let anchorsToHighlight = pageNav.querySelectorAll(
        'a[href="#' + elementId + '"]'
      );
