...
function uploadImageFile(file) {
  ...
  return window.$http
    .post("/images/gallery/upload", formData)
    .then(resp => resp.data);
}
...
function codePlugin() {
  ...
  function showPopup(editor) {
    ...
    let lang = selectedNode.hasAttribute("data-lang")
      ? selectedNode.getAttribute("data-lang")
      : "";
  ...
  window.tinymce.PluginManager.add("codeeditor", function(editor, url) {
    ...
    editor.on("SetContent", function() {
      ...
      $(".CodeMirrorContainer")
        .filter((index, elem) => {
          return (
            typeof elem.querySelector(".CodeMirror").CodeMirror === "undefined"
          );
        })
        .each((index, elem) => {
          codeMirrorContainerToPre($(elem));
...
module.exports = {
  ...
  plugins:
    "image media table textcolor paste link autolink fullscreen imagetools code customhr autosave lists codeeditor",
  imagetools_toolbar: "imageoptions",
  toolbar:
    "undo redo | styleselect | bold italic underline strikethrough superscript subscript | forecolor backcolor | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | table image-insert media link hr | removeformat code fullscreen",
  content_style:
  ...
  setup: function(editor) {
    editor.on(
      "init ExecCommand change input NodeChange ObjectResized",
      editorChange
    );
