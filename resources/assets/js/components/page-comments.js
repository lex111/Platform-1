...
class PageComments {
  ...
  handleAction(event) {
    ...
    if (action === "delete")
      this.deleteComment(actionElem.closest("[comment]"));
  ...
  editComment(commentElem) {
    ...
    commentElem.querySelector("[comment-edit-container]").style.display =
      "block";
    let textArea = commentElem.querySelector(
  ...
  updateComment(event) {
    ...
    window.$http
      .put(window.baseUrl(`/ajax/comment/${commentId}`), reqData)
      .then(resp => {
        ...
        window.$events.emit(
          "success",
          window.trans("entities.comment_updated_success")
        );
  ...
  deleteComment(commentElem) {
    ...
    window.$http.delete(window.baseUrl(`/ajax/comment/${id}`)).then(resp => {
      ...
      window.$events.emit(
        "success",
        window.trans("entities.comment_deleted_success")
      );
      this.updateCount();
    });
  }
  ...
  saveComment(event) {
    ...
    window.$http
      .post(window.baseUrl(`/ajax/page/${this.pageId}/comment`), reqData)
      .then(resp => {
        let newComment = document.createElement("div");
        newComment.innerHTML = resp.data;
        ...
        window.$events.emit(
          "success",
          window.trans("entities.comment_created_success")
        );
        this.resetForm();
        this.updateCount();
      });
  ...
  updateCount() {
    ...
    this.elem.querySelector(
      "[comments-title]"
    ).textContent = window.trans_choice("entities.comment_count", count, {
      count
    });
  }
