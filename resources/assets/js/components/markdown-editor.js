...
class MarkdownEditor {
  ...
  codeMirrorSetup() {
    ...
    // Save draft
    extraKeys[`${metaKey}-S`] = cm => {
      window.$events.emit("editor-save-draft");
    };
    // Save page
    extraKeys[`${metaKey}-Enter`] = cm => {
      window.$events.emit("editor-save-page");
    };
    // Show link selector
    extraKeys[`Shift-${metaKey}-K`] = cm => {
      this.actionShowLinkSelector();
    };
    // Insert Link
    extraKeys[`${metaKey}-K`] = cm => {
      insertLink();
    };
    // FormatShortcuts
    extraKeys[`${metaKey}-1`] = cm => {
      replaceLineStart("##");
    };
    extraKeys[`${metaKey}-2`] = cm => {
      replaceLineStart("###");
    };
    extraKeys[`${metaKey}-3`] = cm => {
      replaceLineStart("####");
    };
    extraKeys[`${metaKey}-4`] = cm => {
      replaceLineStart("#####");
    };
    extraKeys[`${metaKey}-5`] = cm => {
      replaceLineStart("");
    };
    extraKeys[`${metaKey}-d`] = cm => {
      replaceLineStart("");
    };
    extraKeys[`${metaKey}-6`] = cm => {
      replaceLineStart(">");
    };
    extraKeys[`${metaKey}-q`] = cm => {
      replaceLineStart(">");
    };
    extraKeys[`${metaKey}-7`] = cm => {
      wrapSelection("\n```\n", "\n```");
    };
    extraKeys[`${metaKey}-8`] = cm => {
      wrapSelection("`", "`");
    };
    extraKeys[`Shift-${metaKey}-E`] = cm => {
      wrapSelection("`", "`");
    };
    extraKeys[`${metaKey}-9`] = cm => {
  ...
  actionInsertImage() {
    ...
    window.ImageManager.show(image => {
      ...
      let newText =
        "![" + (selectedText || image.name) + "](" + image.thumbs.display + ")";
      this.cm.focus();
