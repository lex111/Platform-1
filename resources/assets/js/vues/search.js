...
let methods = {
  ...
  exactChange() {
    ...
    let matchesTerm = this.search.exactTerms
      .filter(term => {
        return term.trim() !== "";
      })
      .map(term => {
  ...
  typeChange() {
    ...
    let selectedTypes = Object.keys(type)
      .filter(type => {
  ...
  updateSearch(e) {
    ...
    window.location = window.baseUrl(
      "/search?term=" + encodeURIComponent(this.termString)
    );
  },
  ...
  dateChange(optionName) {
    let dateFilter = new RegExp(
      "{\\s?" + optionName + "\\s?:([a-z_\\-0-9]+?)}",
      "gi"
    );
