...
function mounted() {
  ...
  this.$http
    .get(window.baseUrl(`/attachments/get/page/${this.pageId}`))
    .then(resp => {
...
let methods = {
  ...
  fileSortUpdate() {
    this.$http
      .put(window.baseUrl(`/attachments/sort/page/${this.pageId}`), {
        files: this.files
      })
      .then(resp => {
  ...
  deleteFile(file) {
    ...
    this.$http
      .delete(window.baseUrl(`/attachments/${file.id}`))
      .then(resp => {
        this.$events.emit("success", resp.data.message);
  ...
  checkValidationErrors(groupName, err) {
    ...
    if (
      ...
      typeof err.response.data.validation === "undefined"
    )
      return;
  ...
  attachNewLink(file) {
    ...
    this.$http
      .post(window.baseUrl("/attachments/link"), file)
      .then(resp => {
        this.files.push(resp.data);
        this.file = this.newFile();
        this.$events.emit(
          "success",
          trans("entities.attachments_link_attached")
        );
      })
      .catch(err => {
  ...
  updateFile(file) {
    $http
      .put(window.baseUrl(`/attachments/${file.id}`), file)
      .then(resp => {
        ...
        this.$events.emit(
          "success",
          trans("entities.attachments_updated_success")
        );
      })
      .catch(err => {
