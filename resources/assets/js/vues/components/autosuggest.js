...
const methods = {
  ...
  inputKeydown(event) {
    ...
    if (event.keyCode === 40) {
      this.active =
        this.active === this.suggestions.length - 1 ? 0 : this.active + 1;
    } else if (event.keyCode === 38) {
      // Up Arrow
      this.active =
        this.active === 0 ? this.suggestions.length - 1 : this.active - 1;
    } else if (
  ...
  /**
   * Get suggestions from DocsPen. Store and use local cache if already searched.
   * @param {String} input
   * @param {Object} params
   */
