...
<div class="col-md-12 setting-nav nav-tabs">
    ...
    @endif    @if($currentUser->can('user-roles-manage'))
        <a href="{{ baseUrl('/settings/') }}" @if($selected == 'roles') class="selected text-button" @endif><i class="zmdi zmdi-lock-open"></i>{{ trans('settings.roles') }}</a>
    @endif
</div>
@endif